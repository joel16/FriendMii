...
void StatusBar_DisplayBar(void)
{
	...
	pp2d_draw_text(time_x, 3, 0.5f, 0.5f, RGBA8(241, 252, 255, 255), Clock_GetCurrentTime(true));
	Battery_DrawStatus(battery_texture_x, 0);
	
	pp2d_draw_textf(date_x, 3, 0.5f, 0.5f, RGBA8(241, 252, 255, 255), "%d/%d (%s)", Clock_GetDayOfWeek(), Clock_GetMonthOfYear(), Clock_GetCurrentDay(1));
}